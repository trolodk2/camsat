      ...
      final pinZSerwera = _rola == 'admin' ? data['admin_pin'] : data['serwisant_pin'];
			...
			if (_pinController.text == pinZSerwera) {
			  ...
			  globals.rolaZalogowanego = _rola;
				...
				await prefs.setString('rola_zalogowanego', _rola);
...
Future<void> _subskrybujTematyDlaRoli(String rola) async {
  ...
  try {
    ...
    // Subskrypcja z timeoutem – jeśli Firebase nie działa, aplikacja nie wisi
    await Future.any([
      _subskrybujTematy(messaging, rolaCzysta),
      Future.delayed(Duration(seconds: 3)), // po 3 sekundach olewamy
    ]);
...
Future<void> _subskrybujTematy(FirebaseMessaging messaging, String rola) async {
  ...
  if (rola == 'admin') {
    ...
    print("⚠️ Nieznana rola: '$rola'. Tylko 'wszyscy' zostaje.");
  }
}
