  ...
  Future<void> _pobierzDaneFormularza() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        _modelZewnetrzny = dane.modele[_wybranyProducent!]!.first;
      });
    } else {
...
void _ustawDomyslneJednostki(String nazwa) {
  _modelZewController.text = modeleZew[nazwa]?.first ?? '';
  _nrSeryjnyZewController.text = '';
  _jednostkiWew = [
    {
      'model': TextEditingController(text: modeleWew[nazwa]?.first ?? ''),
    ...
    children: [
      ...
      Row(
        children: [
          Expanded(
            child: TextFormField(
              controller: _modelZewController,
