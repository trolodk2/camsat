...
class _DodajZadanieDialogState extends State<DodajZadanieDialog> {
  ...
  int _priorytet = 2;
	XFile? _wybraneZdjecie;

	Future<void> _dodajZadanie() async {
  final uri = Uri.parse('https://cam-sat.pl/klima/api_addTodo.php');
  var request = http.MultipartRequest('POST', uri);

  request.fields['tytul'] = _tytulCtrl.text;
  request.fields['opis'] = _opisCtrl.text;
  request.fields['priorytet'] = _priorytet.toString();

  if (_wybraneZdjecie != null && !kIsWeb) {
    request.files.add(
      await http.MultipartFile.fromPath(
        'zdjecie',
        _wybraneZdjecie!.path,
        filename: path.basename(_wybraneZdjecie!.path),
      ),
    );
  }
            ...
            DropdownButton<int>(
              ...
              items: [
                ...
                DropdownMenuItem(child: Text("Niskie"), value: 3),
						...
						if (_wybraneZdjecie != null) ...[
						  SizedBox(height: 10),
						  Image.network(_wybraneZdjecie!.path, width: 100, height: 100, fit: BoxFit.cover),
						],
          ],
        ),
      ),
...
class EdytujZadanieDialog extends StatefulWidget {
  ...
  EdytujZadanieDialog({required this.zadanie, required this.onZapisane});
...
class _EdytujZadanieDialogState extends State<EdytujZadanieDialog> {
  late TextEditingController _tytulCtrl;
  late TextEditingController _opisCtrl;
  late int _priorytet;

  @override
  void initState() {
    super.initState();
    _tytulCtrl = TextEditingController(text: widget.zadanie.tytul);
    _opisCtrl = TextEditingController(text: widget.zadanie.opis);
    _priorytet = widget.zadanie.priorytet;
  }

  Future<void> _zapiszZmiany() async {
    final url = Uri.parse('https://cam-sat.pl/klima/api_editTodo.php');
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: _zapiszZmiany, child: Text("Zapisz")),
      ],
    );
  }
}
